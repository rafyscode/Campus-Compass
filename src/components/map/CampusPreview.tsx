...
export function CampusPreview({ current }: { current: OccupancySnapshot }) {
  ...
  return <section className="card campus-preview">
    <div className="campus-preview-head">
      <div><span className="eyebrow"><Map size={12} /> Campus Digital Twin</span><h2>{t('Zentraler Campus')}</h2><p>{t('Georeferenzierte 3D-Karte · Mensa-Auslastung auf einen Blick.')}</p></div>
      <div className="campus-preview-actions"><DataSourceBadge mode={current.source} /><Link className="button button-ghost" to="/campus">{t('3D öffnen')} <ArrowUpRight size={14} /></Link></div>
    </div>
